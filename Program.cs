...
namespace XMLParsing
{
  internal class Program
  {
    static void Main(string[] args)
    {
      ...
      //string xmlFilePath = @"C:\\Users\\ersel\\source\\repos\\XMLParsing\\test.xml";
      string xmlFilePath = @"C:\\Users\\ersel\\source\\repos\\XMLParsing\\GetLiveSportsLive.xml";
      ...
      Console.ReadLine();
		...
		{
			string xmlContent = @"<wsdl:definitions 
  ...
  name=""LiveSportsBS"" targetNamespace=""http://tempuri.org/"">
    ...
    <wsdl:service name=""LiveSportsBS"">
      ...
      try
      {
        ...
        doc.Load(@"C:\\Users\\ersel\\source\\repos\\XMLParsing\\test.xml"); // Replace with the actual path to your WSDL XML

        XmlNamespaceManager nsManager = new XmlNamespaceManager(doc.NameTable);
        nsManager.AddNamespace("wsdl", "http://schemas.xmlsoap.org/wsdl/");

        XmlNodeList elements = doc.SelectNodes("//wsdl:*", nsManager);

        foreach (XmlNode element in elements)
        {
          if (element.Prefix == "wsdl")
          {
    ...
    public static void test1()
    {
      ...
      string xmlContent = @"<wsdl:definitions xmlns:wsdl=""http://schemas.xmlsoap.org/wsdl/"" xmlns:wsam=""http://www.w3.org/2007/05/addressing/metadata"" xmlns:wsx=""http://schemas.xmlsoap.org/ws/2004/09/mex"" xmlns:wsap=""http://schemas.xmlsoap.org/ws/2004/08/addressing/policy"" xmlns:msc=""http://schemas.microsoft.com/ws/2005/12/wsdl/contract"" xmlns:wsp=""http://schemas.xmlsoap.org/ws/2004/09/policy"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"" xmlns:soap=""http://schemas.xmlsoap.org/wsdl/soap/"" xmlns:wsu=""http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd"" xmlns:soap12=""http://schemas.xmlsoap.org/wsdl/soap12/"" xmlns:soapenc=""http://schemas.xmlsoap.org/soap/encoding/"" xmlns:tns=""http://tempuri.org/"" xmlns:wsa10=""http://www.w3.org/2005/08/addressing"" xmlns:wsaw=""http://www.w3.org/2006/05/addressing/wsdl"" xmlns:wsa=""http://schemas.xmlsoap.org/ws/2004/08/addressing"" name=""LiveSportsBS"" targetNamespace=""http://tempuri.org/"">
	<wsdl:types>
	...
	<wsdl:service name=""LiveSportsBS"">
      ...
      try
      {
        ...
        // Now you can access elements and attributes using the XmlDocument API
        // For example, let's get the targetNamespace attribute value
        string targetNamespace = xmlDocument.DocumentElement.GetAttribute("targetNamespace");
        Console.WriteLine("Target Namespace: " + targetNamespace);

        // You can also use XPath expressions to retrieve specific elements
        // For example, let's get all the elements named "xs:element"
        XmlNodeList elements = xmlDocument.SelectNodes("//xs:element", GetNamespaceManager(xmlDocument));
        foreach (XmlNode element in elements)
        {
          Console.WriteLine("Element name: " + element.Attributes["name"].Value);
          // You can access other attributes or child elements as needed
        }
      }
    ...
    public static void getXMLWithLinq()
    {

        // Load the WSDL XML into an XDocument
        XDocument wsdlDocument = XDocument.Load(@"C:\\Users\\ersel\\source\\repos\\XMLParsing\\GetLiveSportsLive.xml");

        // Define the XML namespaces
        XNamespace wsdlNamespace = "http://schemas.xmlsoap.org/wsdl/";
        XNamespace xsdNamespace = "http://www.w3.org/2001/XMLSchema";
        XNamespace tnsNamespace = "http://tempuri.org/";
        
        Console.WriteLine("-------------------");
        
        // Extract information from the WSDL using LINQ queries
        var elements = wsdlDocument.Descendants(wsdlNamespace + "element");
        var complexTypes = wsdlDocument.Descendants(xsdNamespace + "complexType");
        var simpleTypes = wsdlDocument.Descendants(xsdNamespace + "simpleType");
        var portTypeOperations = wsdlDocument.Descendants(wsdlNamespace + "portType").Elements(wsdlNamespace + "operation");




      // Example: Print the names of all elements in the schema
      //foreach (var element in elements)
      //{
      //  XAttribute nameAttribute = element.Attribute("name");
      //  if (nameAttribute != null)
      //  {
      //    Console.WriteLine("Element Name: " + nameAttribute.Value);
      //  }
      //}

      // Example: Print the names of all complex types in the schema
      IEnumerable<XElement> desc = null;
      foreach (var complexType in complexTypes)
      {
        XAttribute nameAttribute = complexType.Attribute("name");
        if (nameAttribute != null)
        {
          Console.WriteLine("Complex Type Name: " + nameAttribute.Value);
        }
      }
      ...
      //// Example: Print the names of all simple types in the schema
      //foreach (var simpleType in simpleTypes)
      //{
      //  XAttribute nameAttribute = simpleType.Attribute("name");
      //  if (nameAttribute != null)
      //  {
      //    Console.WriteLine("Simple Type Name: " + nameAttribute.Value);
      //  }
      //}

      //// Example: Print the names of all portType operations in the schema
      //foreach (var operation in portTypeOperations)
      //{
      //  XAttribute nameAttribute = operation.Attribute("name");
      //  if (nameAttribute != null)
      //  {
      //    Console.WriteLine("PortType Operation Name: " + nameAttribute.Value);
      //  }

      //}

    }
    public static void getWSDLTypes()
    {
      string xmlFilePath = @"C:\\Users\\ersel\\source\\repos\\XMLParsing\\GetLiveSportsLive.xml";

      // Load the WSDL document from the file
      XmlDocument xmlDoc = new XmlDocument();
      xmlDoc.Load(xmlFilePath);
      ...
      // Get the <wsdl:types> node
      XmlNode typesNode = xmlDoc.SelectSingleNode("/wsdl:definitions/wsdl:types", nsManager);
      
      if (typesNode != null)
      {
        // You can now access and process the content of <wsdl:types> node as needed
        Console.WriteLine(typesNode.InnerXml);
      }
      else
      {
        Console.WriteLine("<wsdl:types> node not found in the WSDL document.");
      }
    }
    ...
    public static void getElements()
    {
      ...
      // Now you have the XML document loaded from the file and a namespace manager.

      // For example, let's print the "name" attributes of all the "xs:element" nodes:
      XmlNodeList elementNodes = xmlDoc.SelectNodes("//wsdl:types", nsManager);
